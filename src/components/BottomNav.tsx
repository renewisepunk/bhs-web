...
export default function BottomNav() {
  ...
  return (
    <nav className="flex items-center justify-around bg-bhs-black py-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))]">
      {navItems.map((item) => {
        ...
        if (item.action) {
          return (
            <button
              key={item.label}
              onClick={item.action}
              className="flex flex-col items-center gap-1 px-2 transition-colors text-bhs-lime"
            >
              {item.icon}
        ...
        return (
          <Link
            ...
            className={`flex flex-col items-center gap-1 px-2 transition-colors ${
              isActive ? "text-bhs-lime" : "text-white/70 hover:text-white"
            }`}
          >
            {item.icon}
            <span className="text-[10px] tracking-wider font-medium">
